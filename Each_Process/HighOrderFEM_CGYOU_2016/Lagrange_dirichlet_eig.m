function [eig_value, eig_func, A_glob_grad, A_glob_L2] = Lagrange_dirichlet_eig(Lagrange_order, vert, edge, tri, bd, neig)
...
[A_glob_grad, A_glob_L2, bd_dof_idx] = Lagrange_dirichlet_eig_matrix(Lagrange_order, vert, edge, tri, tri2edge, is_edge_bd);

if nt > 1000
    disp('begin to solve eigenvalue problem');
end
neig = min(neig, size(A_glob_grad, 1));
[V, D] = eigs(A_glob_grad, A_glob_L2, neig, 'sm');
...
function [A_glob_grad, A_glob_L2, bd_dof_idx] = Lagrange_dirichlet_eig_matrix(Lagrange_order, vert, edge, tri, tri2edge, is_edge_bd)
[basis, nbasis] = Lagrange_basis(Lagrange_order);
...
A_glob_grad = sparse(ndof, ndof);
A_glob_L2 = sparse(ndof, ndof);
M = sparse(ndof, ndof);
