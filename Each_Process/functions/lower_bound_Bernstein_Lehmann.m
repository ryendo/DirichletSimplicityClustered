function lower_bounds = lower_bound_Bernstein_Lehmann(tri_intval)
    ...
    % Create file names with variable values
    M_filename = sprintf('mat_quotients/M_matrix_n_%d.mat', n);
    K_filename = sprintf('mat_quotients/K_matrix_n_%d.mat', n);
    bbasis_filename = sprintf('mat_quotients/bbasis_n_%d.mat', n);

    % Check if files exist and load matrices
    if isfile(M_filename) && isfile(K_filename) && isfile(bbasis_filename)
        load(M_filename, 'M');
        load(K_filename, 'K');
        load(bbasis_filename, 'bbasis');
    else
        ...
        % Save M and K to ../mat_quotients
        save(M_filename, 'M');
        save(K_filename, 'K');
        save(bbasis_filename, 'bbasis');
    end
    ...
    A1 = sym(zeros(eig_m, eig_m));
    A2 = sym(zeros(eig_m, eig_m));
    A3 = sym(zeros(eig_m, eig_m));
    vs = sym(zeros(eig_m));

    for i = 1:eig_m
        for j = 1:nBasis
            vs(i) = vs(i) + eigenVectors(j,i)*bbasis(j);
        end
    end
    ...
    for i = 1:eig_m
        for j = 1:eig_m
            ...
            A2(i,j) = int(int(Ni * Nj, x, (a*y)/b, (-1+a)*(y-b)/b+a), y, 0, b);
