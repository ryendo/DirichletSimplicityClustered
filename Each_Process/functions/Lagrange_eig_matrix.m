function [A_with_bdry, B_global_with_bdry, A, B_global, bd_dof, all_dof_size] = Lagrange_eig_matrix(Lagrange_order, mesh, eig_N)

    vert = mesh.nodes;
    edge = mesh.edges;
    tri  = mesh.elements;
    bd   = edge(is_edge_on_bdry(mesh),:);
    ...
    node_bdry_list = is_node_on_bdry(mesh,eig_N);
    
    [basis, nbasis] = Lagrange_basis(Lagrange_order);
    bd_dof_edge = zeros((Lagrange_order-1)*eig_N*3,1);
    ...
    M_ip_basis_ij = intval(zeros(nbasis, nbasis));
    M_ip_basis_edge_ij_all = cell(3, 1);
    M_ip_basis_edge1_ij = intval(zeros(nbasis, nbasis));
    M_ip_basis_edge2_ij = intval(zeros(nbasis, nbasis));
    M_ip_basis_edge3_ij = intval(zeros(nbasis, nbasis));
    for i = 1:nbasis
        for j = i:nbasis
    ...
    % M_ip_basis_edge_ij_all{1, 1}
    % M_ip_basis_edge_ij_all{2, 1}
    % M_ip_basis_edge_ij_all{3, 1}
    ...
    for k = 1:nt
        ...
        A_grad = intval(zeros(nbasis, nbasis));
        for i = 1:nbasis
            for j = 1:i
        ...
        for i = 1:3
            ...
            if is_edge_bd(edge_idx(i))==1
                bd_dof_edge((current_bdry_idx*(Lagrange_order-1)+1):(current_bdry_idx+1)*(Lagrange_order-1)) = map_dof_idx_l2g(3+(Lagrange_order-1)*(i-1)+1:3+(Lagrange_order-1)*i);
                current_bdry_idx = current_bdry_idx+1;
            end
        end
        ...
        B_global(map_dof_idx_l2g, map_dof_idx_l2g) = B_global(map_dof_idx_l2g, map_dof_idx_l2g) +A_L2;
    ...
    [all_dof_size,~] = size(A);
    bd_dof = [bd_dof_edge',find(node_bdry_list>0)];
    
    A_with_bdry=A; B_global_with_bdry=B_global;
    A_with_bdry(bd_dof,:)=zeros(size(A_with_bdry(bd_dof,:)));
    A_with_bdry(:,bd_dof)=zeros(size(A_with_bdry(:,bd_dof)));
    B_global_with_bdry(bd_dof,:)=zeros(size(B_global_with_bdry(bd_dof,:)));
    B_global_with_bdry(:,bd_dof)=zeros(size(B_global_with_bdry(:,bd_dof)));
    
    A(bd_dof,:)=[]; A(:,bd_dof)=[];
    B_global(bd_dof,:)=[]; B_global(:,bd_dof)=[];
end
...
for k = 1:nt
    ...
    for j = 1:3
        if edge_status(edge_idx(j)) > 0
            ...
            point = zeros(Lagrange_order + 1, 2);
...
f_for_Hdiv_problem = zeros(ne, Lagrange_order + 1);
for i = 1:ne
