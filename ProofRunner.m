    ...
    function runAlgo1Interval(self, interval, K)
        ...
        for kk = 1:total
            ...
            try
                ...
                fprintf('δ=[%.6g,%.6g]: \n Dtλ2 ∈ [%.12g,%.12g] \n Dtλ3 ∈ [%.12g,%.12g],\n gap ≥ %.12g\n', self.callI_inf(edges(i)),self.callI_sup(edges(i+1)), self.callI_inf(interval_mu(1)), self.callI_sup(interval_mu(1)), self.callI_inf(interval_mu(2)), self.callI_sup(interval_mu(2)), self.callI_inf(interval_mu(2) - interval_mu(1)));
            catch ME
...
methods (Access = private)
    function printAlgo1Preface(self, interval, K)
        ...
        fprintf('    over a direction interval (nearly equilateral triangle).\n');
        fprintf('Parameters:\n');
