...
properties
    ...
    algo2_M (1,1) double = 40        % Steps in x-direction (radial)
    algo2_N (1,1) double = 200       % Steps in theta-direction (angular)
...
methods
    ...
    function setupIntlab(self)
        try
            ...
            elseif exist('my_intlab_config','file')==2
                ...
                warning('No my_intlab_config found. Proceeding with default INTLAB state.');
            end
        catch ME
    ...
    function runAlgo2(self, varargin)
        % RUNALGO2 Executes Algorithm 2 directly in MATLAB using verification_step_2.m.
        %
        % Usage:
        %   runner.runAlgo2();            % Runs all j from 1 to N
        %   runner.runAlgo2(1:10);        % Runs specific j indices
        %   runner.runAlgo2('M', 50);     % Override Grid M temporarily
        ...
        % Default to full range if no list provided
        if isempty(j_list)
            j_list = 1:N;
        end

        if self.verbose
            fprintf('--- Algorithm 2: Domain Monotonicity (Direct Call) ---\n');
            fprintf('Grid: M=%d (x), N=%d (theta)\n', M, N);
            fprintf('Processing %d j-indices. Output prefix: %s\n', numel(j_list), self.algo2_OutFile);
        end
        
        % Ensure INTLAB is ready
        self.setupIntlab();

        % Call the external function directly
        % We pass M and N to avoid hardcoding in the external function
        try
            verification_step_2(j_list, M, N, self.algo2_OutFile);
        catch ME
            error('Algorithm 2 execution failed: %s\n%s', ME.message, getReport(ME));
        end
    ...
    function [up2, lo3] = boundsOnBox(self, r_lo, r_hi, h_lo, h_hi, varargin)
        ...
        y_small = h_lo;
        tri_small = [I_intval(0), I_intval(0); I_intval(1), I_intval(0); x_small, y_small];

        % 2. Construct the "Largest" Triangle (for Lower Bound of lambda3)
        x_large = sqrt(r_hi^2 - h_hi^2);
        y_large = h_hi;
        tri_large = [I_intval(0), I_intval(0); I_intval(1), I_intval(0); x_large, y_large];
