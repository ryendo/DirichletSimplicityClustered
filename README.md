...
> **[Paper B] The Second Dirichlet Eigenvalue is Simple on Every Non-equilateral Triangle, Part II: Nearly Equilateral Triangles** \> https://arxiv.org/abs/2305.14063 *(to appear in Numerische Mathematik)*
...
We split the parameter space of triangles $\Omega$ into three regions:
  ...
  * **Region** $\Omega_{\text{down}}$: Using **Algorithm 2**, we compute high-precision eigenvalue bounds directly to show $(\lambda_2 \le )~ \overline{\lambda}_2 < \underline{\lambda}_3 ~(\le \lambda_3)$.
...
└── my_env_config.m               # Computation environment configuration
````
...
This project requires the **INTLAB** (Interval Laboratory) toolbox and **gmsh**. Please configure the path to your local INTLAB installation. The global variable `INTERVAL_MODE` is use to switch between approximate computation mode and the rigorous computation mode.

Configuration of `my_env_config.m` :

```matlab
% Open my_env_config.m
...
Since `meshio` of python package is used via `pyrun` of MATLAB to load mesh files generated by `gmsh`, one has to install `meshio` in user's python environment and set the python in MATLAB. For example, 
```
pyenv( ...
    Version="/Users/xfliu/python3-standard/bin/python3.11", ...
...
[up2, lo3] = s.boundsAtPoint(intval('0.5'), intval('0.8'),mesh_size_CG, fem_order_CG, mesh_size_CR, isLG, mesh_size_LG, fem_order_LG);
```
