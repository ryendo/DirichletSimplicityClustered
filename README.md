...
Verifies the gap condition over a rectangular region in the parameter space. It computes $\sup(\lambda_2)$ over the "worst-case" geometry for $\lambda_2$ and $\inf(\lambda_3)$ for $\lambda_3$ within the box.

```matlab
% Check box a=[0.49, 0.51], b=[0.80, 0.81]
[up2, lo3] = s.boundsOnBox(intval('0.49'), intval('0.51'), ...
